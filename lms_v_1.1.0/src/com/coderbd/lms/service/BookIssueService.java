...
public class BookIssueService implements BookIssueDao {
    ...
    public void update(BookIssue s) {
        try {
            if (s.getId() != 0) {
                PreparedStatement stmt = conn.prepareStatement("update lms_book_issue SET book_id=?,student_id=?,issue_date=?,expire_date=?,return_date=?,qty=?,status=?  where id=?");
                stmt.setInt(1, s.getBookId());
                stmt.setInt(2, s.getStudentId());
                stmt.setString(3, s.getIssueDate());
                stmt.setString(4, s.getExpireDate());
  ...
  /*  
    public String getBookName(int id){
        ...
        try {
            if (id != 0) {
                PreparedStatement stmt = conn.prepareStatement("select name from lms_book where id=?");
     ...
     public String getStudentName(int id){
        ...
        try {
            if (id != 0) {
                PreparedStatement stmt = conn.prepareStatement("select name from lms_student where id=?");
